...
function calculate(expression) {
  ...
  function comparePrecedence(token1, token2) {
    if (!isOperator(token1)) throw new Error("Invalid token: " + token1);

    if (!isOperator(token2)) throw new Error("Invalid token: " + token2);

    return operators[token1][0] - operators[token2][0];
  }

  // helper function allowing to look up value in last cell in
  ...
  function tokenizeInput(input) {
    ...
    for (i = 0; i < indexesForZeros.length; i++) {
      expression =
  ...
  function shuntingYard(tokens) {
    ...
    // filter empty tokens that might appear
